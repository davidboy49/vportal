...
import { X, Send, Sparkles, RefreshCw, AlertTriangle, Copy, Check } from "lucide-react";
    ...
    for (const match of matches) {
      ...
      if (matchText.startsWith("**") && matchText.endsWith("**")) {
        ...
        parts.push(
          <code
            key={matchIndex}
            className="font-mono text-xs bg-zinc-200/80 dark:bg-zinc-800 px-1 py-0.5 rounded text-pink-650 dark:text-pink-400"
          >
            {matchText.slice(1, -1)}
          </code>
        );
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="opacity-0 group-hover/msg:opacity-100 transition-opacity duration-150 ml-1 shrink-0 p-1 rounded-md text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800"
      title="Copy message"
    >
      {copied ? (
...
export function ChatWidget() {
  ...
  useEffect(() => {
    try {
      ...
      // On parse error, generate a fresh session so logs don't collide
      const fallback = crypto.randomUUID();
      setSessionId(fallback);
      sessionStorage.setItem("vportal_chat_session_id", fallback);
    }
  }, []);
  ...
  // Keyboard shortcut: Ctrl+/ or Alt+C to toggle chat
  useEffect(() => {
  ...
  const handleSendMessage = useCallback(
    async (text: string) => {
      if (!text.trim() || isLoading || charOver) return;
      ...
      try {
        ...
        if (response.status === 429) {
          ...
          const msg =
            errorData?.choices?.[0]?.message?.content ||
        ...
        if (replyContent) {
          if (
            replyContent.includes("GEMINI_API_KEY") ||
            replyContent.includes("HF_TOKEN")
          ) {
    ...
    [messages, isLoading, sessionId, user, charOver]
  );
  ...
  return (
    ...
    >
        ...
        <div className="relative mb-4 flex h-[540px] w-[380px] flex-col overflow-hidden rounded-2xl border border-white/20 bg-white/80 shadow-2xl backdrop-blur-xl transition-all duration-300 dark:border-zinc-800/40 dark:bg-zinc-950/80 sm:w-[420px] animate-in slide-in-from-bottom-5 duration-300 ease-out">
              ...
              <div className="flex h-full flex-col justify-center space-y-5 text-center px-3 py-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-zinc-400 dark:text-zinc-600 mt-1">
                    Tip: Press <kbd className="px-1 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 font-mono text-[10px] border border-zinc-200 dark:border-zinc-700">Ctrl+/</kbd> to open/close
                  </p>
                </div>
              ...
              <div className="space-y-4">
                {messages.map((message, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`flex max-w-[88%] items-start space-x-2 ${isUser ? "flex-row-reverse space-x-reverse" : "flex-row"} group/msg`}>
                        {!isUser && (
                    ...
                    <div className="flex items-start space-x-2">
                      ...
                      <div className="rounded-2xl rounded-tl-none bg-zinc-100/90 dark:bg-zinc-900/90 px-4.5 py-3.5 border border-zinc-200/60 dark:border-zinc-800/40 shadow-sm">
                        <div className="flex space-x-1.5 items-center justify-center h-2 w-9">
                          <div className="h-1.5 w-1.5 animate-bounce rounded-full bg-zinc-400 dark:bg-zinc-500 [animation-delay:-0.3s]" />
                          <div className="h-1.5 w-1.5 animate-bounce rounded-full bg-zinc-400 dark:bg-zinc-500 [animation-delay:-0.15s]" />
                          <div className="h-1.5 w-1.5 animate-bounce rounded-full bg-zinc-400 dark:bg-zinc-500" />
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {hasPromptedSetup && (
            <div className="flex items-center space-x-2 bg-amber-50/80 border-t border-amber-200 px-4 py-2.5 text-xs text-amber-800 dark:bg-amber-950/20 dark:border-amber-900/30 dark:text-amber-400">
              ...
              <span>
                API key required. Open <code>.env.local</code> and set <code>GEMINI_API_KEY</code>.
              </span>
            </div>
          )}
            ...
            <div className="flex items-center space-x-2">
              <div className="relative flex-1">
                <input
                  ...
                  disabled={isLoading}
                  className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-4 py-2.5 text-sm outline-none transition-all focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500/20 dark:border-zinc-800 dark:bg-zinc-900/50 dark:text-zinc-100 dark:focus:border-blue-400 dark:focus:bg-zinc-900 disabled:opacity-50 pr-14"
                />
                {/* Character counter */}
                {charCount > 0 && (
                  <span
                    className={`absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-mono transition-colors ${
                      charOver
                        ? "text-rose-500"
                        : charWarning
                        ? "text-amber-500"
              ...
              <button
                type="submit"
                disabled={isLoading || !inputValue.trim() || charOver}
      ...
      <button
        ...
        title="Open VPortal Support Chat (Ctrl+/)"
      >
          ...
          {isOpen ? (
            ...
            <>
              <Image
                ...
                width={36}
                height={36}
                className="h-9 w-9 object-contain rounded-full transform-gpu transition-all duration-300 hover:rotate-6"
                unoptimized
              />
              {messages.length === 0 && (
